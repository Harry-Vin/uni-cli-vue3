...
**AI提示词：**
“请创建一个使用 Vue 3 和 Vite 的 Uniapp 项目。项目名称定为 `my-uniapp-base-project`。

1.  **项目初始化:** 生成初始项目结构。
2.  **`vite.config.js` 配置:**
    *   确保 `uni()` 插件已正确导入和使用。
    *   添加一个示例的 H5 开发代理配置，代理 `/api` 到 `http://localhost:3000`。

请提供 `vite.config.js` 的完整内容，并说明创建项目时的命令。
”
